...
const ToFeedButton = () => {
  ...
  const topicPath = getTopicPath(pathname);

  return (
    <a href={topicPath} className={buttonVariants({ variant: "ghost" })}>
      <ChevronLeft className="h-4 w-4 mr-1" />
      {topicPath === "/" ? "Back home" : "Back to community"}
    </a>
  );
};

const getTopicPath = (pathname: string) => {
